...
SKIP_KEYS = [ "type", "subtype", "channel", "id", "mic", "mod",
               "sequence_num", "message_type", "exception", "raw_msg" ]
...
mappings.update({
    "Consumption": {
        ...
        "config": {
            "name": "Consumption",
            "state_class": "total_increasing",
            "unit_of_measurement": "count"  # swap to gal/m³/kWh later
        }
    },
    "EndpointID": {
        "device_type": "sensor",
        "object_suffix": "endpoint_id",
        "config": {"name": "Endpoint ID"}
    },
    "EndpointType": {
        "device_type": "sensor",
        "object_suffix": "endpoint_type",
        "config": {"name": "Endpoint Type"}
    },
    "ProtocolID": {
        "device_type": "sensor",
        "object_suffix": "protocol_id",
        "config": {"name": "Protocol ID"}
    },
    "MeterType": {
        "device_type": "sensor",
        "object_suffix": "meter_type",
        "config": {"name": "Meter Type"}
    },
    "Tamper": {
        "device_type": "binary_sensor",
        "object_suffix": "tamper",
        "config": {"name": "Tamper", "device_class": "tamper"}
    },
})
...
# Track which devices already have a diagnostics entity published
diagnostics_published = set()

def publish_diagnostics_config(mqttc, model, object_id, base_topic):
    """Publish discovery for a per-device diagnostics sensor with attributes."""
    unique = f"{object_id}-diagnostics"
    path = "/".join([args.discovery_prefix, "sensor", object_id, unique, "config"])

    # Avoid republishing discovery too often
    now = time.time()
    ttl = discovery_timeouts.get(path, 0)
    if ttl > now:
        pass
    else:
        discovery_timeouts[path] = now + args.discovery_interval
        ...
        config = {
            "name": "Diagnostics",
            "unique_id": unique,
            "state_topic": f"{base_topic}/diagnostics/state",
            "json_attributes_topic": f"{base_topic}/diagnostics/attributes",
            "icon": "mdi:information-outline",
            "entity_category": "diagnostic",
            "device": { "identifiers": [object_id], "name": object_id, "model": model, "manufacturer": "rtl_433" },
        }
        if args.force_update:
            config["force_update"] = "true"
        if args.expire_after:
        ...
        mqttc.publish(path, json.dumps(config), retain=args.retain)
        diagnostics_published.add(object_id)

    # Ensure the state topic exists so HA shows the entity
    mqttc.publish(f"{base_topic}/diagnostics/state", "ok", retain=args.retain)
...
def bridge_event_to_hass(mqttc, topic_prefix, data):
    ...
    if args.ids and "id" in data and data.get("id") not in args.ids:
        logging.debug("Device (%s) is not in the desired list of device ids: %s",
                      data.get("id"), args.ids)
        return
    ...
    # Unknowns → diagnostics attributes
    unknown = {}
    for k, v in data.items():
        if k in mappings or k in SKIP_KEYS:
            continue
        if k in ("model", "id", "time", "type", "subtype", "channel"):
            continue
        # (optional) skip noisy RF meta:
        # if k.lower() in ("protocol", "rssi", "snr", "noise", "repeat", "flags", "mic", "mod"):
        #     continue
        unknown[k] = v

    if unknown:
        publish_diagnostics_config(mqttc, model, device_id, base_topic)
        mqttc.publish(f"{base_topic}/diagnostics/attributes",
                      json.dumps(unknown),
                      retain=args.retain)
        skipped_keys = [k for k in skipped_keys if k not in unknown]
